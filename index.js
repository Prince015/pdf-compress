...
const pdfPath = "./original/Scorecards.pdf";
const outputDir = "./out";
...
async function readFile(pdfPath) {
  try {
    ...
    // console.log(data);
    console.log(
      `pdf size before compression : ${Math.round(
    ...
    await convertPdfToImage(`./compressed/compressed-${uuid}.pdf`, outputDir);
  } catch (err) {
    console.error(err);
  }
}
...
async function convertPdfToImage(pdfPath, outputPath) {
  const opts = {
    ...
    outdir: outputPath, // output directory path
    prefix: path.basename(pdfPath, path.extname(pdfPath)),
    // page: null, // Specify the page number here to convert a specific page, otherwise null to convert all pages
  };
  try {
    console.log("Converting PDF to image...");
    try {
      await pdftoimage(pdfPath, opts)
        .then(function () {
    ...
    const numberOfPages = parseInt(5024);
    ...
    for (let i = 1; i <= numberOfPages; i++) {
      pages.push(
        `./out/${path.basename(pdfPath, path.extname(pdfPath))}-${String(
          i
        ).padStart(String(numberOfPages).length, "000")}.jpg`
      );
    }
    ...
    for(const page of pages){
      const data = await sharp(page)
      .jpeg({ mozjpeg: true, quality: 50 })
      .toBuffer();
      fs.writeFileSync(page, data);
    ...
    // size = ppt size 16:9
    const width = info?.width_in_pts || 792;
    const height = info?.height_in_pts || 612;
